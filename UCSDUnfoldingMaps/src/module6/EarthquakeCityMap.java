...
public class EarthquakeCityMap extends PApplet {
	...
	// Markers for each earthquakehttp://marketplace.eclipse.org/marketplace-client-intro?mpc_install=945
	...
	public void setup() {		
		...
		size(900, 700, OPENGL);
		if (offline) {
		    map = new UnfoldingMap(this, 50, 50, 800, 600, new MBTilesMapProvider(mbTilesString));
	    ...
	    for (PointFeature p : airports) {
	    	airportMarkers.add(new AirportMarker(p));
	    }
	...
	private void sortAndPrint(int numToPrint)
	{
		...
		EarthquakeMarker[] m = new EarthquakeMarker[array.length];

		for (int i = 0; i < array.length; i++) {
		    m[i] = (EarthquakeMarker)array[i];
		}
		
		int len = (m.length < numToPrint ? m.length: numToPrint);
		// use selection sort
		int indexMain;
		for(int i=0; i < m.length-1; i++)
		{
			...
			for(int j=i+1; j < m.length; j++) {
				if( m[j].compareTo(m[i]) < 1) {
					indexMain = j;
				}
			}
			EarthquakeMarker temp = m[indexMain];
			m[indexMain] = m[i];
			m[i] = temp;
		...
		for ( int pos=1; pos < m.length ; pos++ ) {
			currInd = pos ;
			while ( currInd > 0 &&  m[currInd].compareTo(m[currInd-1]) < 0 ) {

				EarthquakeMarker temp = m[currInd];
				m[currInd] = m[currInd-1];
				m[currInd-1] = temp;
	...
	private void printAirports() {
		for (Marker airport: airportMarkers) {
			System.out.println(airport.getStringProperty("name") + " " + airport.getStringProperty("city") + " " + airport.getStringProperty("code"));
		}
	}
